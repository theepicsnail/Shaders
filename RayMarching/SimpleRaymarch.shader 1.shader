Shader "snail/RayMarching/SimpleRaymarch 1" {
	Properties {
...
float4 snoise_grad(float3 v)
{
    ...
    return 42.0 * float4(grad, dot(m4, px));
			...
			float3 compute_distance(float3 p) {
				float4 r = snoise_grad(p*.05);
				return length(r.xyz * r.z *.1);
			/*
	...
	// length(pos-z);
			...
			fragmentOutput frag(fragmentInput i, fixed facing : VFACE) {
				...
				float3 wpos = lerp(_WorldSpaceCameraPos, i.worldpos, facing > 0);
				...
				float3 normal = estimate_normal(wpos);
				float intensity = abs(dot(_WorldSpaceLightPos0, normal)) / (dist_hit.x*dist_hit.x) * 10;
